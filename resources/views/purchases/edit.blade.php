            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" class="form-control @error('bill_date') is-invalid @enderror"
                           id="bill_date" name="bill_date" value="{{ old('bill_date', $purchase->bill_date instanceof \Carbon\Carbon ? $purchase->bill_date->format('Y-m-d') : $purchase->bill_date) }}" required>
                    @error('bill_date')
                        <span class="text-danger small">{{ $message }}</span>
                    @enderror
                </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            ...
            <textarea class="form-control @error('notes') is-invalid @enderror"
                      id="notes" name="notes" rows="3" placeholder="Any additional notes...">{{ old('notes', $purchase->notes) }}</textarea>
            @error('notes')
                <span class="text-danger small">{{ $message }}</span>
            @enderror
        </div>
...
@php
    ...
    $existingItems = $purchase->items->map(function ($item) {
        return [
            ...
            'serials' => $item->serialNumbers->pluck('serial_number')->implode(', '),
        ];
    })->values();

    $oldPurchaseItems = old('items') ? collect(old('items'))->map(function ($item) use ($productNameMap) {
        return [
            'product_id' => (string) ($item['product_id'] ?? ''),
...
<script id="initialItemsJson" type="application/json">@json($oldPurchaseItems)</script>
<script>
$(document).ready(function() {
    var rowIndex = 0;

    var products = JSON.parse(document.getElementById('purchaseProductsJson').textContent || '[]');
    var initialItems = JSON.parse(document.getElementById('initialItemsJson').textContent || '[]');
